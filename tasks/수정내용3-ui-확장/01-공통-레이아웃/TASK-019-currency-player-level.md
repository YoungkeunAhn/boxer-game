...
> 정정(구현 완료 기준): 본문의 'v6→v7'·`boxer-game.save.v7`은 P2가 v6을 쓴다는 전제였으나, P2(TASK-014~018)가 저장 무변경이라 코드 실제 버전이 `SCHEMA_VERSION=5`였다. 따라서 실제 범프는 **SCHEMA v5→v6**(`boxer-game.save.v6`), **BALANCE 6→7**(경험치 곡선)로 구현했다. 옛 v5는 `LEGACY_SAVE_KEYS`에 추가해 삭제 없이 `legacy` 안내.
