...
- [ ] 애니메이션 키 매핑(`boxer_left_jab`·`boxer_right_straight`·`boxer_left_hook` 등), 손 선택 규칙(TASK-007) 준수
- [ ] 모션 상태: 대기/공격/피격/회피/사망/스킬, 회피 모션을 몬스터 공격·회피 결과와 연동
- [ ] 인파이터 톤: 묵직한 타격·화면 흔들림·GUARD/훅·어퍼/피해 감소 강조
- [ ] 아웃복서 톤: 잔상·빠른 스텝·MISS/COUNTER/스트레이트 궤적 강조
- [ ] 전투 텍스트 연출: 데미지 숫자·콤비네이션·스킬명·MISS/GUARD/COUNTER·보스 WARNING
- [ ] 성능: WebView 60fps 목표, 가벼운 CSS/transform 기반 우선
- [ ] 등급분류: 과격·출혈·부상·잔혹 연출 배제 확인
- [ ] 연출이 전투 로직/난수/타이밍을 바꾸지 않음(표시 계층 전용) 확인
- [ ] E2E/스모크: 연출이 핵심 정보 가리지 않음·360px 깨짐 없음 항목 추가
- [ ] `node tools/check.mjs full` + `npm run e2e` 통과
