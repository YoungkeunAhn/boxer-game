...
### 구현 가정값 (TASK-009, `src/game/constants.ts`)

가정: 아래 임시값은 TASK-013에서 확정한다. 코드(`constants.ts`)·테스트와 같은 값을 유지한다. 그로기는 보스 전투 런타임 전용 값이라 저장하지 않는다(`CombatRuntime`의 `groggyGauge`/`groggyMax`/`groggyUntil`, 비저장 → `SCHEMA_VERSION` 5 유지). 새 전투 수식이므로 `balanceVersion`은 7.
