...
| **Implement** | 단일 에이전트 | 현재 작업 트리에 계획대로 구현 + colocated 테스트 추가, `check.mjs fast`가 green이 될 때까지 자체 반복 | ✅ (커밋 X) |
...
Implement 단계의 에이전트는 **현재 체크아웃된 작업 트리에 그대로** 구현한다. 브랜치를 새로 만들거나 전환하지 않는다 — 어떤 브랜치를 쓸지는 사람이 워크플로우 실행 전에 결정한다. 커밋·푸시도 하지 않는다(구현·검증·리뷰까지만). 스테이징·커밋·푸시는 사람이 `deploy`로 처리한다.
...
- **브랜치·커밋은 사람이.** 워크플로우는 현재 작업 트리에서 구현·검증·리뷰만 한다. 브랜치 분기와 스테이징·커밋·푸시는 사람이 처리한다(`deploy` 스킬).
