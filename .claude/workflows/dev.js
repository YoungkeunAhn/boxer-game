...
5. 보고 전 CLAUDE.md "변경 후 체크리스트"의 각 항목을 직접 대조한다(순수 함수·클램프·스테이지 접근·버전 범프·테스트 동기화·게이트·MVP 경계). 어긋난 게 있으면 고친 뒤 다시 게이트를 돌린다.
커밋은 하지 마라(스테이징/커밋은 사람이 한다). 마지막에: 작업한 브랜치 이름, 수정한 파일 목록, 올린 버전(schema/balance), check.mjs 최종 결과, 체크리스트 대조 결과, 남은 TODO/가정, 다음 태스크 제안을 요약해 반환하라.`,
