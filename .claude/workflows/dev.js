export const meta = {
  ...
  phases: [
    ...
    { title: 'Implement', detail: '계획대로 현재 작업 트리에 구현하고 check.mjs fast로 자체 검증(green까지)' },
...
- Karpathy 원칙: 요구된 것만 최소 변경으로. 불필요한 추상화·미리 만드는 일반화·요청 안 된 기능 추가 금지. 모든 변경에 검증 가능한 성공 기준을 둘 것.`

const PLAN_SCHEMA = {
  type: 'object',
  additionalProperties: false,
  required: ['summary', 'impact', 'files', 'versionBumps', 'tests', 'risks'],
  properties: {
    summary: { type: 'string', description: '이 태스크가 무엇을 바꾸는지 2~3문장' },
    impact: {
      type: 'object',
      additionalProperties: false,
      required: ['touched', 'reverseDeps', 'riskLevel'],
      properties: {
        touched: {
          type: 'array',
          items: { type: 'string' },
          description: '직접 바꾸는 핵심 모듈(formulas/stages/types(SaveData)/constants/save 등)',
        },
        reverseDeps: {
          type: 'array',
          items: { type: 'string' },
          description: '그 변경이 파급되는 역의존처(호출 사용처·스토어 직렬화·콜로케이트 테스트·버전/문서). CLAUDE.md "변경 전 영향도" 체인 참고',
        },
        riskLevel: {
          type: 'string',
          enum: ['high', 'medium', 'low'],
          description: 'high=세이브 호환성/핵심 진행 직결, medium=표시·진행 영향, low=국소',
        },
      },
    },
    files: {
...
영향도(impact)를 반드시 먼저 분석하라: 직접 바꾸는 핵심 모듈을 정하고, 그 변경이 파급되는 역의존처(Grep으로 호출 사용처·스토어 직렬화·콜로케이트 테스트 추적)와 버전/문서 갱신 필요 여부를 찾아 위험도를 매긴다. CLAUDE.md의 "변경 전 영향도" 커플링 체인을 기준으로 삼아라.

계획만 세우고 코드는 수정하지 마라. 영향도(impact), 어떤 파일을 어떻게 바꿀지, 스키마/밸런스 버전을 올려야 하는지, 어떤 테스트를 추가할지, 가정/리스크가 무엇인지 구조화해서 반환하라.`,
...
구현 절차(현재 체크아웃된 작업 트리에 그대로 작업한다 — 브랜치를 새로 만들거나 전환하지 마라):
1. 계획대로 src/ 파일을 수정하고 colocated 테스트를 추가/갱신한다.
2. 변경한 모든 파일을 인자로 'node tools/check.mjs fast "<바뀐 파일1>" "<바뀐 파일2>" ...' 를 실행한다.
3. 실패하면 원인을 고치고 green이 될 때까지 반복한다.
4. 보고 전 CLAUDE.md "변경 후 체크리스트"의 각 항목을 직접 대조한다(순수 함수·클램프·스테이지 접근·버전 범프·테스트 동기화·게이트·MVP 경계). 어긋난 게 있으면 고친 뒤 다시 게이트를 돌린다.
커밋은 하지 마라(스테이징/커밋은 사람이 한다). 마지막에: 수정한 파일 목록, 올린 버전(schema/balance), check.mjs 최종 결과, 체크리스트 대조 결과, 남은 TODO/가정, 다음 태스크 제안을 요약해 반환하라.`,
